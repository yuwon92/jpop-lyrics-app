
# 🎵 J-Pop 가사 번역 & 단어장

일본어 가사를 공부하기 위한 데스크톱 앱입니다.  
가사를 붙여넣으면 히라가나를 자동으로 생성해주고, 번역을 입력하며 단어를 저장할 수 있습니다.

---

## 설치 방법

[Releases 페이지](https://github.com/yuwon92/jpop-lyrics-app/releases/latest)에서 운영체제에 맞는 파일을 다운로드하세요.

### Windows

1. `J-Pop Vocab Setup x.x.x.exe` 다운로드 후 실행
2. "Windows의 PC 보호" 파란 경고 창이 뜨면 **추가 정보 → 실행** 클릭
   - 개인 개발 앱이라 서명 인증서가 없어서 뜨는 경고입니다
3. 설치가 끝나면 바탕화면/시작 메뉴의 **J-Pop Vocab**으로 실행

### Mac

1. `J-Pop Vocab-x.x.x.dmg` 다운로드 후 열기
2. 앱 아이콘을 **응용 프로그램(Applications)** 폴더로 드래그
3. 터미널(Terminal)을 열고 아래 명령어 실행 — 다운로드한 앱에 대한 macOS 차단(격리 속성)을 해제합니다:

   ```bash
   xattr -dr com.apple.quarantine "/Applications/J-Pop Vocab.app"
   ```

4. 응용 프로그램 폴더에서 **J-Pop Vocab** 실행

> 실행 시 "예기치 않게 종료되었습니다" 메시지가 뜨면 아래 명령어로 서명을 붙인 뒤 다시 실행하세요:
>
> ```bash
> codesign --force --deep --sign - "/Applications/J-Pop Vocab.app"
> ```

---

## 주요 기능

- **히라가나 자동 변환** — 일본어 가사를 붙여넣으면 한자 → 히라가나 자동 변환
- **한글 발음 토글** — Claude AI를 이용해 히라가나 발음을 한글 발음으로 변환 (첫 변환 1회만 API 사용, 이후 토글은 로컬 캐시 사용)
- **가사 편집** — 원문 / 발음(히라가나 or 한글) / 번역을 한 줄씩 대응해서 보여주는 3단 구조
- **단어 즉시 저장** — 가사 보다가 모르는 단어를 드래그 후 `Ctrl+E`로 즉시 단어장에 추가
  - 선택한 텍스트가 단어 입력 필드에 자동으로 채워짐
  - 발음(히라가나)은 kuroshiro로 자동 변환, 뜻은 Claude AI로 자동 번역 (API 키 필요)
  - 저장 전에 발음·뜻 직접 수정 가능
  - 텍스트 선택 시 근처에 `Ctrl+E로 단어 추가` 힌트 팝업 표시
- **단어장** — 가사 보면서 모르는 단어를 바로 저장
  - 단어 카드에 히라가나 발음 표시 (한자 단어에만)
  - 저장된 단어 수정 가능 (✎ 버튼)
  - 전체 단어장 / 노래별 단어장 보기
  - 즐겨찾기(★) 기능
  - 셔플 모드
  - 낱말카드 (플립 애니메이션, 키보드 조작, 뒷면 발음 표시)
- **문법 노트** — 문법 포인트를 저장·관리하는 전용 탭
  - 전체 / 노래별 보기, 즐겨찾기, 수정, 삭제
  - 가사 분석 패널의 `+` 버튼으로 즉시 저장하거나 탭에서 직접 추가
- **가사 줄 분석** *(베타)* — 가사 편집 화면에서 줄 우측 `?` 버튼을 누르면 Claude Sonnet이 해당 줄의 번역과 문법을 분석해 인라인으로 표시
  - 직역 / 의역, 핵심 단어 카드, 문법 포인트
  - 단어 카드의 `+` 버튼으로 단어장에 즉시 저장
  - 분석 결과는 로컬에 캐시되어 같은 줄 재분석 시 API를 다시 호출하지 않음
  - > ⚠ **베타 기능:** AI가 생성하는 내용으로 부정확한 번역, 오류, 이상한 설명이 포함될 수 있습니다. 학습 참고용으로만 활용하세요.
- **컬러 테마** — 설정 화면에서 파스텔 / 화이트 / 다크 테마 선택 가능, 선택 유지됨
- **노래 저장/관리** — 번역한 노래 목록을 홈 화면에서 관리

## 스크린샷

<img width="1468" height="938" alt="스크린샷 2026-06-23 225400" src="https://github.com/user-attachments/assets/b5fdd099-911e-4bf5-a7b9-cbc84d13d6b9" />
<img width="1465" height="945" alt="스크린샷 2026-06-23 225409" src="https://github.com/user-attachments/assets/6bb0ef52-1e8c-43aa-a3a8-b77d1666c39f" />
<img width="1465" height="932" alt="스크린샷 2026-06-23 225417" src="https://github.com/user-attachments/assets/0023c4bf-897c-48e7-ae48-a4ec4caccedc" />
<img width="1466" height="943" alt="스크린샷 2026-06-23 225510" src="https://github.com/user-attachments/assets/ee8d1d08-0338-4564-a797-519efe3d7a7a" />

---

## 기술 스택

| 분류 | 사용 기술 |
|------|-----------|
| 프레임워크 | Electron 29 + React 18 + TypeScript |
| 빌드 | electron-vite |
| 히라가나 변환 | kuroshiro + kuroshiro-analyzer-kuromoji |
| 한글 발음 변환 | Claude AI API (claude-haiku, 사용자 API 키 필요) |
| 데이터 저장 | JSON 파일 (`%APPDATA%\jpop-lyrics-app\`) |
| UI | 레트로 픽셀 스타일 (PF Stardust 3.0 / Noto Sans JP) |

---

## 개발 환경 실행

```bash
# 의존성 설치
npm install

# 개발 서버 실행 (cmd 권장)
npm run dev
```

> **Windows 주의사항**  
> 시스템 환경변수에 `ELECTRON_RUN_AS_NODE=1`이 설정되어 있으면 앱이 실행되지 않습니다.  
> 시스템 환경변수에서 해당 항목을 삭제한 뒤 실행하세요.

---

## 빌드 (배포용 exe 생성)

관리자 권한 cmd에서 실행:

```bash
npm run package
```

`dist/` 폴더에 포터블 exe 파일이 생성됩니다.

---

## 한글 발음 기능 사용법

가사 편집 화면 우측 상단의 `[ひ → 가]` 버튼을 누르면 발음 모드를 전환할 수 있습니다.

1. **첫 번째 토글 시** — Anthropic API 키 입력 모달이 표시됩니다
   - [Anthropic Console](https://console.anthropic.com/)에서 API 키를 발급받으세요
   - 입력한 키는 기기에 암호화되어 로컬 저장되며 외부로 전송되지 않습니다
2. **변환 완료 후** — 한글 발음이 DB에 저장되어 이후 토글은 즉시 전환됩니다 (API 재호출 없음)
3. **히라가나 ↔ 한글 발음** 을 언제든지 자유롭게 전환할 수 있습니다

> **API 비용** — Claude Haiku 기준 곡당 약 $0.002 (약 3원) 수준입니다.

---

## 데이터 위치

저장된 노래와 단어장은 아래 경로에 JSON 파일로 저장됩니다:

```
%APPDATA%\jpop-lyrics-app\jpop-lyrics-data.json
```

다른 PC로 이전하려면 이 파일을 복사하면 됩니다.
